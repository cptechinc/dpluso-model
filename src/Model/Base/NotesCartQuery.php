...
use \NotesCart as ChildNotesCart;
use \NotesCartQuery as ChildNotesCartQuery;
use \Exception;
use \PDO;
 ...
 * @method     ChildNotesCartQuery orderBySequence($order = Criteria::ASC) Order by the sequence column
 * @method     ChildNotesCartQuery orderByNote($order = Criteria::ASC) Order by the note column
 * @method     ChildNotesCartQuery orderByForm($order = Criteria::ASC) Order by the form column
 * @method     ChildNotesCartQuery orderByDate($order = Criteria::ASC) Order by the date column
 * @method     ChildNotesCartQuery orderByTime($order = Criteria::ASC) Order by the time column
 *
 * @method     ChildNotesCartQuery groupBySessionid() Group by the sessionid column
 * @method     ChildNotesCartQuery groupByItemid() Group by the itemid column
 * @method     ChildNotesCartQuery groupBySequence() Group by the sequence column
 * @method     ChildNotesCartQuery groupByNote() Group by the note column
 * @method     ChildNotesCartQuery groupByForm() Group by the form column
 * @method     ChildNotesCartQuery groupByDate() Group by the date column
 * @method     ChildNotesCartQuery groupByTime() Group by the time column
 *
 * @method     ChildNotesCartQuery leftJoin($relation) Adds a LEFT JOIN clause to the query
 * @method     ChildNotesCartQuery rightJoin($relation) Adds a RIGHT JOIN clause to the query
 * @method     ChildNotesCartQuery innerJoin($relation) Adds a INNER JOIN clause to the query
 *
 * @method     ChildNotesCartQuery leftJoinWith($relation) Adds a LEFT JOIN clause and with to the query
 ...
 * @method     ChildNotesCart requireOneBySequence(int $sequence) Return the first ChildNotesCart filtered by the sequence column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildNotesCart requireOneByNote(string $note) Return the first ChildNotesCart filtered by the note column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildNotesCart requireOneByForm(string $form) Return the first ChildNotesCart filtered by the form column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
...
abstract class NotesCartQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = '\\NotesCart', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$sessionid, $itemid, $sequence] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ($con === null) {
            $con = Propel::getServiceContainer()->getReadConnection(NotesCartTableMap::DATABASE_NAME);
        }
        ...
        if ((null !== ($obj = NotesCartTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1]), (null === $key[2] || is_scalar($key[2]) || is_callable([$key[2], '__toString']) ? (string) $key[2] : $key[2])]))))) {
     ...
     *
     * @return ChildNotesCart A model object, or null if the key is not found
     */
    protected function findPkSimple($key, ConnectionInterface $con)
    {
        $sql = 'SELECT sessionid, itemid, sequence, note, form, date, time FROM notes_cart WHERE sessionid = :p0 AND itemid = :p1 AND sequence = :p2';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            /** @var ChildNotesCart $obj */
            $obj = new ChildNotesCart();
            $obj->hydrate($row);
            NotesCartTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1]), (null === $key[2] || is_scalar($key[2]) || is_callable([$key[2], '__toString']) ? (string) $key[2] : $key[2])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(NotesCartTableMap::COL_SESSIONID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(NotesCartTableMap::COL_ITEMID, $key[1], Criteria::EQUAL);
        $this->addUsingAlias(NotesCartTableMap::COL_SEQUENCE, $key[2], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            $cton1 = $this->getNewCriterion(NotesCartTableMap::COL_ITEMID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $cton2 = $this->getNewCriterion(NotesCartTableMap::COL_SEQUENCE, $key[2], Criteria::EQUAL);
            $cton0->addAnd($cton2);
            $this->addOr($cton0);
        }
    ...
    public function filterBySequence($sequence = null, $comparison = null)
    {
        if (is_array($sequence)) {
            ...
            if (isset($sequence['min'])) {
                $this->addUsingAlias(NotesCartTableMap::COL_SEQUENCE, $sequence['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($sequence['max'])) {
                $this->addUsingAlias(NotesCartTableMap::COL_SEQUENCE, $sequence['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    {
        if ($notesCart) {
            $this->addCond('pruneCond0', $this->getAliasedColName(NotesCartTableMap::COL_SESSIONID), $notesCart->getSessionid(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(NotesCartTableMap::COL_ITEMID), $notesCart->getItemid(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond2', $this->getAliasedColName(NotesCartTableMap::COL_SEQUENCE), $notesCart->getSequence(), Criteria::NOT_EQUAL);
    ...
    public function doDeleteAll(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            NotesCartTableMap::clearRelatedInstancePool();
    ...
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $criteria->setDbName(NotesCartTableMap::DATABASE_NAME);
        ...
        return $con->transaction(function () use ($con, $criteria) {
            ...
            NotesCartTableMap::removeInstanceFromPool($criteria);
